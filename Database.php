...
public static function create_teacher($postArray, $token){
	...
	if($postArray['role'] =='teacher'){
		...
		$statement->bindValue(':teacherId', $teacherId);
		$statement->bindValue(':school', $postArray['schoolName']);
		$statement->bindValue(':schoolBoard', $postArray['schoolBoardName']);
		$statement->execute();
	}
}
//Function to create a new user in the database
private static function create_user($postArray, $token){
	$encrypted_password = password_hash($postArray['password'],PASSWORD_DEFAULT);
...
public static function retrieve_email($username){
	...
	public static function vedio_id()
	{
	...
	//function to retrieve videos on clicking the particular link

	public static function view_videos($names)
	{
		$link=array();
		$query1 = "SELECT * FROM videos WHERE name = :name";
		$statement = self::getDB()->prepare($query1);
		$statement->bindValue(':name', $names);
		$statement->execute();
		$results = $statement->fetchAll();
		foreach ($results as $result)
			$link[$result['name']] = $result['link'];
		return $link;
